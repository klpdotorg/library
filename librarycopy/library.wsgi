...
db=web.database(dbn='postgres',user='',pw='',db='library')
...
sqlstatements={"selectdistrict":"select distinct dist.id,dist.name from vw_tb_schools_boundary as dist, vw_tb_schools_boundary as blck, vw_tb_schools_boundary as clst, vw_institution as inst where clst.parent_id=blck.id and blck.parent_id=dist.id and clst.id=inst.boundary_id and inst.id in (select distinct klp_school_id from lang_agg)",
	...
	"selectborrow":"select trans_year,class as clas,getmonth(split_part(issue_date,\'/\',2)) as month,school_name,count(klp_child_id) from libentry where flag is not null and klp_school_id=$schlid and flag is not null group by klp_school_id,month,trans_year,class,school_name",
	"selectclass":"select distinct class as clas from vw_totalstudents where isdigit(class)=\'t\' and klp_school_id=$schlid order by class",
	"selectyear":"select distinct year from level_agg where class is not null and klp_school_id=$schlid",
	"selecttotalstudents":"select class as clas,count from vw_totalstudents where isdigit(class)='t' and klp_school_id=$schlid"
}
